...
export const StudentPortal: React.FC = () => {
    ...
    // Helper to check if student has paid for a given trimester
    const hasPaidForTrimester = (trimestre: string): boolean => {
        if (!student) return false;
        const fullClassName = student.serie ? `${student.classe} ${student.serie}` : student.classe;
        const monthlyFee = settings.accounting.classFees[fullClassName] || 0;
        const match = trimestre.match(/^S?(\d+)/);
        const trimesterNumber = match ? parseInt(match[1], 10) : 0;
        const required = monthlyFee * 3 * trimesterNumber;
        return (student.totalPaid || 0) >= required;
    };
